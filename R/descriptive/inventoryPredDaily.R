...
rs <- dbSendQuery(con, "select * from inventory_video_section;")
data <- fetch(rs, n=10)
...
#timeSeq$date <- as.Date(rownames(timeSeq))
...
p2 <- ggplot(relGrowth) +
    ...
    geom_text(aes(x=ch,y=growth/2-0.01,label=ch),size=3,show.legend=F) + 
    scale_fill_manual(values=gCol1) +
    theme(
...
jpeg("fig/invHistGrowth.jpg",width=pngWidth,height=pngHeight)
print(p1)
print(p2, vp = vp)
dev.off()
