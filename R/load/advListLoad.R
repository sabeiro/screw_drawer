...
for(k in 1:length(varLoopIn)){
    ...
    colnames(aggr1)[colnames(aggr1)==varList[iN]] <- 'var1'
    colnames(aggr1)[colnames(aggr1)==varList[oN]] <- 'var2'
    impsSum <- as.matrix(xtabs(imps ~ var1 + var2,aggr1))
    clickSum <- as.matrix(xtabs(click ~ var1 + var2,aggr1))
