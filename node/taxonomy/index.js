...
var processTaxonomyOPMLNode = (nodeIndex, next) =>{
    if(nodeIndex == numElements){
	statusUpdate("OK","taxonomy","taxonomy:update:opml:ended",_mode)
	next()
    }
    else{
	elem = $(elements[nodeIndex])
	statusUpdate("OK","taxonomy","taxonomy:update:opml:category",{"current":(nodeIndex+1),"total": numElements, "category":elem.attr("text")})
	...
	// EMPTY OPML FILE: SET ROOT ID
	if(BK_category_name == "Self-Classification" && BK_category_ID == "")
	    BK_category_ID = _seats[_mode].masterNodeID
	...
	//var BK_category_mutex_children = "false"
	...
	if(BK_URL_rule.length > 0)
	    BK_URL_rule_length = 2 // GENERATE A PAIR OF URL RULES
	...
	SEQUENCE.create()
		...
		if(BK_category_ID == "" || BK_category_ID == null){
			...
			SEQUENCE.create()
				...
				statusUpdate("OK","taxonomy","taxonomy:update:opml:category:created",BK_category_name)
		...
		if(BK_URL_rule.length > 2){
		    ...
		    // GENERATE URL RULE EXAXT == FALSE
		    var BK_URL_rule_ID = BK_URL_rule_IDs[0]
		    BK_URL_rule_IDs.splice(0,1)

		    var rnd_url_rule_id = Math.floor(Math.random()*1000000)

		    opml_rules.push({
			"id": BK_URL_rule_ID,
			"name" : BK_category_ID + ":URL:" + BK_category_name + "_1" + "_" + rnd_url_rule_id,
			"type": "url",
			"urls": BK_URL_rule,
			"referrer": "false",
			"exact": "false",
		    ...
		    // GENERATE URL RULE EXACT == TRUE
		    var BK_URL_rule_ID = BK_URL_rule_IDs[0]
		    BK_URL_rule_IDs.splice(0,1)

		    opml_rules.push({
			"id": BK_URL_rule_ID,
			"name" : BK_category_ID + ":URL:EXACT:" + BK_category_name + "_1" + "_" + rnd_url_rule_id,
			"type": "url",
			"urls": BK_URL_rule,
			"referrer": "false",
			"exact": "true",
